...
async function connectCheck (host, protocol, port) {
  return new Promise((resolve, reject) => {
    if (protocol === 'udp') {
      reject(Error('UDP support not yet implemented.'))
    }
    try {
      const client = new net.Socket()
      client.on('error', (error) => {
        reject(Error(`Failed to connect: host=${host}, protocol=${protocol}, port=${port}. ${error}`))
      })
      client.on('timeout', (error) => {
        reject(Error(`Connection time-out: host=${host}, protocol=${protocol}, port=${port}. ${error}`))
      })
      client.connect(port, host, () => {
        resolve(`Connected to host=${host}, protocol=${protocol}, port=${port}`)
      })
    } catch (error) {
      reject(Error(`Failed to connect to host=${host}, protocol=${protocol}, port=${port}. ${error}`))
    }
  })
}
